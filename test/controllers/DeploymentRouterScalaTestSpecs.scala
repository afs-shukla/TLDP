  ...
  "update deploymets tests" should {
   ...
   /* "Search deployments" in {
    ...
    "find by status deployments" in {
      ...
      val deploy1=Await.result(depRepo.create("DEP001",ur1,app2,new java.sql.Timestamp(new Date().getTime),"SIT","SUCCESS","Dummy Deployment","fixVersion"),Duration.Inf)
      val deploy2=Await.result(depRepo.create("DEP002",ur2,app3,new java.sql.Timestamp(new Date().getTime),"UAT","SUCCESS","Dummy Deployment","fixVersion"),Duration.Inf)
      println("SIT Deployment:"+deploy1)
      println("UAT deployment:"+deploy2)

      val Some(result) = route(app, FakeRequest(GET, s"/findByStatus/${"SUCCESS"}"))
      println("Result:" + result)
      println(contentAsJson(result))
      status(result) mustEqual OK
      println(contentAsJson(result))
