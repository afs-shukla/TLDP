   ...
   /* "Insert New Rows in " in {
    ...
    "Update deployment " in {
     val timestamp=new java.sql.Timestamp(new Date().getTime)
      ...
      val deploy1=Await.result(deployRepo.create("DEP001",ur1,app2,timestamp,"SIT","SUCCESS","Dummy Deployment"),Duration.Inf)
      val deploy2=Await.result(deployRepo.create("DEP002",ur2,app3,timestamp,"UAT","SUCCESS","Dummy Deployment"),Duration.Inf)
      println("SIT Deployment:"+deploy1)
      println("UAT deployment:"+deploy2)
      val deploy=Deploy(1,"DEP001UPdate",ur1,app2,timestamp,"SIT","SUCCESS","Dummy Deployment")
