...
/**
  ...
  */
class JiraScalaTestSpec  extends PlaySpec with GuiceOneAppPerTest  with BeforeAndAfterEach with LazyLogging {
    ...
    "An item " should {
      ...
      "Insert New Rows in Svn Details" in {
        ...
        val jira1=Await.result(jiraRepo.create(deploy1, "JIRA-0320", "Creating New accunt for clinet", "Completed","10.44.22"), Duration.Inf)
        val jira2=Await.result(jiraRepo.create(deploy1, "JIRA-0321", "Creatign GID", "Completed","10.44.22"), Duration.Inf)
        val jira3=Await.result(jiraRepo.create(deploy1, "JIRA-0322", "Correcting SSI Details", "Completed","10.44.22"), Duration.Inf)
        val jira4=Await.result(jiraRepo.create(deploy1, "JIRA-0323", "Adding new account number for me", "Completed","10.44.22"), Duration.Inf)
        val alljira=Await.result(jiraRepo.findJiraDetailsByDeplId(deploy1),Duration.Inf)
        logger.info("JIRA Details =======:"+alljira)

        alljira.size mustBe 4
